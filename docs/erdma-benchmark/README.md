...
一个镜像同时含 perftest 和 NCCL 测试所需的一切（拉取 ~400MB，解压 ~690MB）：
...
| NCCL | `libnccl` **2.27.7+cuda12.9**（Blackwell/sm_120 必需，本镜像自己从 NVIDIA CUDA repo 装）+ 全部 `nccl-tests` `*_perf` 二进制 |
...
| 文件 | 用途 |
|------|------|
| [`nccl-slim/`](./nccl-slim) | 测试镜像 `:nccl-slim` 构建（`Dockerfile` + `assemble.sh`，派生自 `:nccl`，拉取 ~400MB）|
