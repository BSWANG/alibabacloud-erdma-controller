...
func TestNewRateLimiterUsesIndependentBucketsForERDMAAPIs(t *testing.T) {
	r := NewRateLimiter(nil)
	for _, api := range []string{
		APIDescribeInstances,
		APIDescribeNetworkInterfaces,
		APICreateNetworkInterface,
		APIModifyNetworkInterfaceAttribute,
		APITagResources,
		APIDescribeInstanceTypes,
		APIAttachNetworkInterface,
		APIDescribeInstanceAttribute,
	} {
		assert.Contains(t, r.store, api)
		assert.NotSame(t, r.store[""], r.store[api])
	}
}
